        ...
        if (!productExists) {
            ...
            if (req.files && req.files.length > 0) {
                for (let i = 0; i < req.files.length; i++) {
                    ...
                    const originalExtension = path.extname(req.files[i].originalname).toLowerCase();
                    const isPng = originalExtension === '.png';
                    // Generate a unique filename using timestamp or UUID
                    const uniqueFilename = `${Date.now()}-${Math.floor(Math.random() * 1000)}${path.extname(req.files[i].originalname)}`;
                     const relativePath = path.join('uploads', 'product-images', uniqueFilename);
                   const resizedImagePath = path.join('public', 'uploads', 'product-images', uniqueFilename.replace(/\.[^/.]+$/, ".jpg"));
...
const deleteSingleImage = async (req, res) => {
    try {
        ...
        console.log("Image name:", imageNameToServer, "Product ID:", productIdToServer);
